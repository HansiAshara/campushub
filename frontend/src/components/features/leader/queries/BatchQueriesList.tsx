...
import type { BatchQueryResponse } from "../../../../api/batchQueryService";

interface BatchQueriesListProps {
    batchId: number;
}

export function BatchQueriesList({ batchId }: BatchQueriesListProps) {
    const [queries, setQueries] = useState<BatchQueryResponse[]>([]);
    const [loading, setLoading] = useState(true);

    const fetchQueries = async () => {
        try {
            const res = await batchQueryService.getByBatch(batchId);
            setQueries(res.data);
        } catch (error) {
    ...
    const handleResolve = async (queryId: number) => {
        try {
            await batchQueryService.resolve(queryId);
                ...
                >
                    <div style={{ flex: 1 }}>
                        <div style={{ display: "flex", alignItems: "center", gap: 10, marginBottom: 8 }}>
                            <span style={{
                                background: q.category === "Module Request" ? "#FEE2E2" : q.category === "Coordinator Nomination" ? "#DBEAFE" : "#F3F4F6",
                                color: q.category === "Module Request" ? "#991B1B" : q.category === "Coordinator Nomination" ? "#1E40AF" : "#374151",
